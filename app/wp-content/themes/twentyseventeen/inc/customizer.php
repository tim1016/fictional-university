 ...
 */
function reisavvy_customize_register( $wp_customize ) {
	...
	$wp_customize->selective_refresh->add_partial(
		'blogname', array(
			...
			'render_callback' => 'reisavvy_customize_partial_blogname',
		)
	);
	$wp_customize->selective_refresh->add_partial(
		'blogdescription', array(
			'selector'        => '.site-description',
			'render_callback' => 'reisavvy_customize_partial_blogdescription',
		)
	);
	...
	$wp_customize->add_setting(
		'colorscheme', array(
			...
			'sanitize_callback' => 'reisavvy_sanitize_colorscheme',
		)
	);
	...
	$wp_customize->add_setting(
		'page_layout', array(
			'default'           => 'two-column',
			'sanitize_callback' => 'reisavvy_sanitize_page_layout',
			'transport'         => 'postMessage',
		)
	);
	...
	for ( $i = 1; $i < ( 1 + $num_sections ); $i++ ) {
		...
		$wp_customize->add_control(
			'panel_' . $i, array(
				...
				'type'            => 'dropdown-pages',
				'allow_addition'  => true,
				'active_callback' => 'reisavvy_is_static_front_page',
			)
		);
		...
		$wp_customize->selective_refresh->add_partial(
			'panel_' . $i, array(
				'selector'            => '#panel' . $i,
				'render_callback'     => 'reisavvy_front_page_section',
				'container_inclusive' => true,
			)
		);
	}
}
add_action( 'customize_register', 'reisavvy_customize_register' );
 ...
 */
function reisavvy_sanitize_colorscheme( $input ) {
 ...
 * @return void
 */
function reisavvy_customize_partial_blogname() {
	bloginfo( 'name' );
}
 ...
 * @return void
 */
function reisavvy_customize_partial_blogdescription() {
 ...
 */
function reisavvy_is_static_front_page() {
 ...
 */
function reisavvy_is_view_with_layout_option() {
